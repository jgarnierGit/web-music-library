<template>
    <v-row justify="center">
        <v-col>
            <audio ref="audioPlayer" controls preload="auto" autoplay="true" crossOrigin="anonymous"
        ...
        <v-col>
            <v-tooltip location="top" origin="auto" text="Change preset">
                <template v-slot:activator="{ props }">
                    <v-btn v-bind="props" :icon=mdiSwapHorizontal @click="loadMilkdropPreset()" />
                </template>
            </v-tooltip>
            <v-tooltip location="top" origin="auto" text="Toggle Visualizer">
                <template v-slot:activator="{ props }">
                    <v-btn v-bind="props" :icon="viewerIcon" @click="toggleViewer()" />
                </template>
            </v-tooltip>
        </v-col>
        <v-spacer />
